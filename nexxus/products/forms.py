...
class ProductForm (forms.Form):
    ...
    def clean(self):
        ...
        if available and (units is None or units < 1):
            raise forms.ValidationError('Las unidades deben ser especificadas y no negativas cuando el producto está disponible.')
        
        # Validar que no haya unidades disponibles cuando el producto no está disponible
        if not available and units and units > 0:
            raise forms.ValidationError('No puede haber unidades disponibles cuando el producto no está disponible.')
